...
class Botonpanicoeditarestado extends CI_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->helper('url');
                $this->load->model('botonpanicoestado_model');
                 $this->load->model('generic_model');
		$this->load->database();
	}

	//redirect if needed, otherwise display the user list
	function index($botonpanicoid)
	{
           
            $botonpanico = array();
//			$result = $this->botonpanicoestado_model->getAlertaByUser($botonpanicoid);
            
            $result = $this->botonpanicoestado_model->getAlertaByUser($botonpanicoid);
            
          
			if ($result != FALSE){
				$botonpanico = array('result' => $result);
			}else {
				$botonpanico = array('result' => '');
			}	

            $this->load->view('admin/edit_boton',$botonpanico);       
	}
...
//                'alergia' => $this->input->post('numerovictimas'),
            ...
            $res = $this->botonpanicoestado_model->saveBotonPanicoEstado($data);
            
            if($res){
                echo Open('table',array('class'=>'table table-striped'));
                    echo Open('tr',array('class'=>'success'));
                        echo tagcontent('td', $this->input->post('longitud'));
                        echo tagcontent('td', $this->input->post('latitud'));
//                        echo tagcontent('td', $this->input->post('numerovictimas'));
	...
	function getById($id)
	{
            $res['enfermedad'] = $this->botonpanicoestado_model->getById($id);
	...
	function edit_alerta()
        { 
             ...
             $estado2 = $this->input->post('estado');
                ...
                $res = $this->generic_model->update( 'botonpanico', array('estado'=>$estado, 'contadorestados'=>$contadorestados), array('idB'=>$idB) );
//                ////////////
//            
//            $res = $this->botonpanicoestado_model->edit($data,$this->input->post('idB'));
